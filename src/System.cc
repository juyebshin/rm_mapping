        ...
        mbDeactivateLocalizationMode(false), mbStart(!bUseViewer), mMatQ(Q)
{
    ...
    mptLocalMapping = new thread(&RM_SLAM::LocalMapping::Run,mpLocalMapper);

    //Initialize the Loop Closing thread and launch
    mpLoopCloser = new LoopClosing(mpMap, mpKeyFrameDatabase, mpVocabulary, mSensor!=MONOCULAR);
    mptLoopClosing = new thread(&RM_SLAM::LoopClosing::Run, mpLoopCloser);
...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    list<RM_SLAM::KeyFrame*>::iterator lRit = mpTracker->mlpReferences.begin();
...
//     vector<RM_SLAM::MapPoint*> vpMPs = mpMap->GetAllMapPoints();
