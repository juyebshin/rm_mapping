...
void Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims, uint8_t* M1, uint8_t* M2){
...
#endif
  computeDisparity(p_support,tri_1,disparity_grid_1,grid_dims,desc1.I_desc,desc2.I_desc,0,D1,M1);
  computeDisparity(p_support,tri_2,disparity_grid_2,grid_dims,desc1.I_desc,desc2.I_desc,1,D2,M2);
...
void Elas::computeDisparity(vector<support_pt> p_support,vector<triangle> tri,int32_t* disparity_grid,int32_t *grid_dims,
                            uint8_t* I1_desc,uint8_t* I2_desc,bool right_image,float* D, uint8_t* M) {
      ...
      for (int32_t u=max((int32_t)A_u,0); u<min((int32_t)B_u,width); u++){
        if (!param.subsampling || u%2==0) {
          ...
          for (int32_t v=min(v_1,v_2); v<max(v_1,v_2); v++){
            if(v < 333) continue;
            if(M[v*width+u] == 0 || M[v*width+u] == 255) continue;
